...
namespace KombfuscaWebManager.Controllers
{
    public class DocsController : Controller
    {
        ...
        {
            var cup = _context.Cups.Where(c => c.Id == cupId).Include(c => c.Periods).Include(c => c.Assignments).FirstOrDefault();

            int nPages = cup.Periods.Count;
            string gameName = cup.Name;
            string[] periodsDesc = cup.Periods.Select(p => p.Description).ToArray();

            var counters = cup.Assignments.ToArray();
            ...
            using (MemoryStream streamFinal = new MemoryStream())
            {
                ...
                foreach (var counter in counters)
                {
                    using (MemoryStream streamTemp = new MemoryStream())
                    {
                        ...
                        Paragraph title = new Paragraph(gameName).SetFont(fontObj).SetFontSize(16).SetFontColor(iText.Kernel.Colors.ColorConstants.BLACK).SetTextAlignment(iText.Layout.Properties.TextAlignment.CENTER);

                        float posX = 0;
                        float posY = 420;

                        title.SetFixedPosition(posX, posY, 595);
